...
_setup_user_() {
	...
	useradd --shell /usr/bin/zsh --gid "${DEVBOX_GROUP_ID}" --uid "${DEVBOX_USER_ID}" --home-dir "${DEVBOX_USER_HOME}" --create-home "${DEVBOX_USER}"
...
bootstrap() {
	apt update && apt install -y openssh-server sudo vim iproute2
