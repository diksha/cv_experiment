...
resource "helm_release" "SecretProviderClass" {
  ...
  force_update     = true
  values = [templatefile("${path.module}/files/cirrus-service-secret-provider-class-values.yaml",
    {
...
resource "argocd_application" "service" {
  ...
  spec {
    ...
    source {
      path            = "terraform/modules/aws/shared/charts/cirrus"
      ...
      helm {
        ...
        parameter {
          name         = "args"
          value        = join(" ", var.command_args)
          force_string = false
        }
    
        # value_files  = ["values-test.yml"]
        values = templatefile("${path.module}/files/cirrus-service-values.yaml",
          {
            ...
            COMMAND_ARGS         = join(" ", var.command_args)
        })
