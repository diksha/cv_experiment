...
type KinesisVideoArchivedMediaAPI interface {
	...
	GetMediaForFragmentList(ctx context.Context, params *kvam.GetMediaForFragmentListInput, optFns ...func(*kvam.Options)) (*kvam.GetMediaForFragmentListOutput, error)
}
...
func streamIdentiferFromName(streamName string) streamIdentifier {
	return streamIdentifier{
...
func newKVSClient(
	...
	requiredAPIEndpoints := [...]kinesisvideotypes.APIName{
		...
		kinesisvideotypes.APINameGetMediaForFragmentList,
	}
...
func (client *kvsClient) GetMediaForFragmentList(ctx context.Context, fragments []string) (io.ReadCloser, error) {
	endpoint, ok := client.EndpointURLS[kinesisvideotypes.APINameGetMediaForFragmentList]
	if !ok {
		return nil, fmt.Errorf("Did not initialize data endpoint URL for %q", kinesisvideotypes.APINameGetMediaForFragmentList)
	}

	endpointResolver := kvam.EndpointResolverFromURL(endpoint)
	input := &kvam.GetMediaForFragmentListInput{
		Fragments:  fragments,
		StreamARN:  client.StreamID.ARN(),
		StreamName: client.StreamID.Name(),
	}
	...
	output, err := client.
		KinesisVideoArchivedMediaClient.
		GetMediaForFragmentList(ctx, input, kvam.WithEndpointResolver(endpointResolver))
	if err != nil {
		return nil, fmt.Errorf("failed call to GetMediaForFragmentList: %w", err)
	}
	return output.Payload, nil
}

func (client *kvsClient) GetMediaForFragment(ctx context.Context, fragmentNumber string) (io.ReadCloser, error) {
	return client.GetMediaForFragmentList(ctx, []string{fragmentNumber})
}
