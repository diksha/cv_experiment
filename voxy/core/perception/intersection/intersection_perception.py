...
class IntersectionPerception:
    ...
    def __call__(self, frame):
        ...
        if self.intersections is None:
            ...
            self.intersections = camera_config.intersections
        for track_id, intersection_polygon in enumerate(self.intersections):
            actor_id = get_actor_id_from_actor_category_and_track_id(
                track_id, ActorCategory.INTERSECTION
            )
            actors.append(
                Actor(
                    category=ActorCategory.INTERSECTION,
                    track_id=actor_id,
