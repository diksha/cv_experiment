...
def pose_nms(
    bboxes, bbox_scores, bbox_ids, pose_preds, pose_scores, areaThres=0
):
    """
    ...
    while human_scores.shape[0] != 0:
        ...
        # Delete humans who have more than matchThreds keypoints overlap and
        # high similarity
    ...
    for j in range(len(pick)):
        ...
        final_result.append(
            {
                "box2": [xmin, ymin, xmax - xmin, ymax - ymin],
                "box": [
                    bbox[0],
                    bbox[1],
                    bbox[2] - bbox[0],
                ...
                "box_score": max(ori_bbox_scores[merge_id].flatten().tolist()),
                "keypoints": (merge_pose - 0.3).tolist(),
                "kp_score": merge_score.tolist(),
                "proposal_score": torch.mean(merge_score)
                + bbox_scores_pick[j]
                + 1.25 * max(merge_score),
                "idx": idx,
                "box_idx": int(idx)
                if type(idx) in [float, int]
                else int(
                    ori_bbox_ids[merge_id].tolist()[
                        int(torch.argmax(ori_bbox_scores[merge_id]))
                    ]
                ),
            }
        )
...
def p_merge_fast(ref_pose, cluster_preds, cluster_scores, ref_dist):
    ...
    final_pose = torch.mul(cluster_preds, normed_scores.repeat(1, 1, 2)).sum(
        dim=0
    )
...
def get_parametric_distance(i, all_preds, keypoint_scores, ref_dist):
    ...
    final_dist = torch.sum(score_dists, dim=1) + mu * torch.sum(
        point_dist, dim=1
    )
