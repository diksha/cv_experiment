...
class AlbumentationsTransforms:
    ...
    def image_texture(self):
        """Composes a transform to augment the image texture
        Returns:
            transforms(list): A composed transform of blurred and compressed images
        """
        augmented = A.Compose(
            [
                A.OneOf(
                    [
                        ...
                        A.GaussianBlur(
                            blur_limit=(3, 7), sigma_limit=0, p=0.2
                        ),
                    ],
                    p=0.5,
                )
