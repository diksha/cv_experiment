...
class ActorMachine:
    ...
    MAX_TIME_WINDOW_TO_STOP_FOR_AISLE_S = 5

    INCIDENT_THRESH_LOW = 1.2
    INCIDENT_THRESH_MED = 1.6
    INCIDENT_THRESH_HIGH = 2.0
    ...
    def __init__(
        ...
        if params is not None:
            ...
            self.MAX_TIME_WINDOW_TO_STOP_FOR_AISLE_S = params.get(
                "max_time_window_to_stop_for_aisle_s",
                self.MAX_TIME_WINDOW_TO_STOP_FOR_AISLE_S,
            )

        # ensure that INCIDENT_THRESH_LOW < INCIDENT_THRESH_MED < INCIDENT_THRESH_HIGH
        self.INCIDENT_THRESH_MED = min(
            self.INCIDENT_THRESH_MED, self.INCIDENT_THRESH_HIGH
        )
        self.INCIDENT_THRESH_LOW = min(
            self.INCIDENT_THRESH_LOW, self.INCIDENT_THRESH_MED
        )

        self.MAX_TIME_WINDOW_TO_STOP_FOR_AISLE_MS = (
            1000 * self.MAX_TIME_WINDOW_TO_STOP_FOR_AISLE_S
        )

        self.last_known_stop_timestamp_ms = None
        self.state_event_start_timestamp_ms = None
        self.state_event_end_timestamp_ms = None
        self.velocity_magnitude_at_aisle_crossing = None
    ...
    def _initialize_state_machine(self) -> None:
        ...
        transitions = [
            ...
            {
                "trigger": "pit_aisle_crossed",
                "source": "start",
                "dest": "incident",
                "conditions": "_is_pit_going_too_fast_at_aisle",
            },
            {
                "trigger": "pit_aisle_crossed",
                ...
                "dest": "incident",
                "conditions": [
                    "_is_pit_going_too_fast_at_aisle",
    ...
    def transition_state_event(
        ...
        if isinstance(state_event_message, State):
            ...
            self.velocity_magnitude_at_aisle_crossing = (
                state_event_message.normalized_speed
                if state_event_message.normalized_speed is not None
                else 0.0
            )
            self.pit_aisle_crossed(
                velocity=self.velocity_magnitude_at_aisle_crossing,
    ...
    # Conditionals
    def _is_pit_going_too_fast_at_aisle(self, velocity, **kwargs) -> bool:
...
class NoStopAtAisleMachine(BaseStateMachine):
    NAME = "no_stop_at_aisle_end"
    INCIDENT_TYPE_NAME = "Speeding At End of Aisle"
    INCIDENT_TYPE_ID = "NO_STOP_AT_END_OF_AISLE"
    ...
    # Cache Configs
    MAX_CACHE_SIZE = 20  # max 50 actors in a scene
    ...
    def get_relevant_actor(
        ...
        if (
            ...
            and state_event_message.event_type == EventType.PIT_EXITING_AISLE
        ):
            return RelevantActorInfo(
