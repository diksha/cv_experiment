...
class ActorMachine:
    max_no_bumpcap_s = (
        5  # At least 5 seconds on the floor with no bump cap is a violation
    )
    ...
    def __init__(
        ...
        if params is not None:
            self.max_no_bumpcap_s = params.get(
                "max_no_bumpcap_s", self.max_no_bumpcap_s
            )

        self.max_no_bumpcap_ms = self.max_no_bumpcap_s * 1000

        self.first_seen_not_wearing_bumpcap_timestamp_ms = None
    ...
    def _initialize_state_machine(self):
        states = ["start", "not_wearing_bumpcap_on_floor", "incident"]

        transitions = [
            {
                "trigger": "update_person_state",
                "source": "start",
                "dest": "not_wearing_bumpcap_on_floor",
                "conditions": [
                    "_is_not_wearing_bumpcap",
                    "_is_not_associated",
                ],
            },
            ...
            {
                "trigger": "update_person_state",
                "source": ["not_wearing_bumpcap_on_floor", "incident"],
                "conditions": [
                    "_is_not_wearing_bumpcap",
                    "_is_not_associated",
                ],
            ...
            },
            {
                ...
                "source": "not_wearing_bumpcap_on_floor",
                "conditions": [
    ...
    def transition_state_event(
        ...
        conditions (eg. person_is_wearing_bumpcap, person_is_associated, etc) and
        ...
        not_wearing_bumpcap_on_floor and condition satisifies.
        ...
        self.update_person_state(
            person_is_wearing_bumpcap=state_event_message.person_is_wearing_hard_hat,
    ...
    def _is_not_wearing_for_greater_than_time_threshold(
        ...
        """Checks if the threshold is reached for bumpcap on floor
        ...
        return (
            self.state_event_end_timestamp_ms
            - self.first_seen_not_wearing_bumpcap_timestamp_ms
            > self.max_no_bumpcap_ms
        )

    def _is_not_wearing_bumpcap(
        self, person_is_wearing_bumpcap, **kwargs
    ) -> bool:
        """Check if the person is wearing bumpcap

        Args:
            person_is_wearing_bumpcap (_type_): Tells whether person is wearing
            bumpcap or not (bool)

        Returns:
            bool: True if wearing bumpcap else False
        """
        # Check only False and ignore None
        return person_is_wearing_bumpcap is False
    ...
    def _get_incident(self, incident_list, **kwargs) -> None:
        incident_list.append(
            Incident(
                ...
                start_frame_relative_ms=self.first_seen_not_wearing_bumpcap_timestamp_ms,
    ...
    def on_enter_not_wearing_bumpcap_on_floor(self, **_) -> None:
        """This function is called whenever the transition machine enters the
        not_wearing_bumpcap_on_floor state and we note the respective state_event_timestamp
...
class BumpCapViolationMachine(BaseStateMachine):
    ...
    NAME = "bump_cap"
    INCIDENT_TYPE_ID = "BUMP_CAP"
    INCIDENT_TYPE_NAME = "Bump Cap"
    ...
    def transition_actor_machine(
        ...
        Args:
            actor_info (RelevantActorInfo): Actor unique identification number
