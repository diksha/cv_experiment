...
class ActorMachine:
    MAX_NO_VEST_S = 5  # At least 5 seconds on the floor with no safety vest to be considered a violation
    ...
    def __init__(
        ...
        if params is not None:
            self.MAX_NO_VEST_S = params.get(
                "max_no_vest_s", self.MAX_NO_VEST_S
            )

        self.MAX_NO_VEST_MS = self.MAX_NO_VEST_S * 1000

        self.first_seen_not_wearing_vest_timestamp_ms = None
    ...
    def _initialize_state_machine(self):
        states = ["start", "not_wearing_vest_on_floor", "incident"]

        transitions = [
            {
                "trigger": "update_person_state",
                "source": "start",
                "dest": "not_wearing_vest_on_floor",
                "conditions": [
                    "_is_not_wearing_safety_vest",
                    "_is_not_associated",
                ],
            },
            ...
            {
                "trigger": "update_person_state",
                "source": ["not_wearing_vest_on_floor", "incident"],
                "conditions": [
                    "_is_not_wearing_safety_vest",
                    "_is_not_associated",
                ],
            ...
            {
                "trigger": "update_person_state",
                "source": ["start", "not_wearing_vest_on_floor", "incident"],
                "dest": "start",
            },
            {
                "trigger": "check_incident",
                "source": "not_wearing_vest_on_floor",
                "conditions": [
    ...
    def _is_not_wearing_for_greater_than_time_threshold(
        ...
        return (
            self.state_event_end_timestamp_ms
            - self.first_seen_not_wearing_vest_timestamp_ms
            > self.MAX_NO_VEST_MS
        )

    def _is_not_wearing_safety_vest(
        self, person_is_wearing_safety_vest, **kwargs
    ) -> bool:
        # Check only False and ignore None
        return person_is_wearing_safety_vest is False
    ...
    def _get_incident(self, incident_list, **kwargs) -> None:
        incident_list.append(
            Incident(
                ...
                start_frame_relative_ms=self.first_seen_not_wearing_vest_timestamp_ms,
    ...
    def on_enter_not_wearing_vest_on_floor(self, **kwargs) -> None:
        self.first_seen_not_wearing_vest_timestamp_ms = (
...
class SafetyVestViolationMachine(BaseStateMachine):

    NAME = "safety_vest"
    INCIDENT_TYPE_ID = "SAFETY_VEST"
    INCIDENT_TYPE_NAME = "Safety Vest"
    PRE_START_BUFFER_MS = 11000
