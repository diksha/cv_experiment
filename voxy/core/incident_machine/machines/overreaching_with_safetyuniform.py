...
class ActorMachine:
    ...
    # Configs: Initialized values from overreaching.py and safetyvest.py
    max_allowed_bad_reach_with_vest_duration_s = 0.5  # Using min of the two
    ...
    def __init__(
        ...
        if params is not None:
            self.max_allowed_bad_reach_with_vest_duration_s = params.get(
                "max_allowed_bad_reach_with_vest_duration_s",
                self.max_allowed_bad_reach_with_vest_duration_s,
            )

        self.max_allowed_bad_reach_with_vest_duration_ms = (
            self.max_allowed_bad_reach_with_vest_duration_s * 1000
        )

        self.first_seen_not_overreaching_with_vest_timestamp_ms = None
        ...
        transitions = [
            {
                ...
                "source": "start",
                "dest": "reaching_badly_with_wearing_uniform_on_floor",
                "conditions": [
                    "_is_reaching_badly",
            ...
            },
            {
                ...
                "source": "reaching_badly_with_wearing_uniform_on_floor",
                "dest": "incident",
                "conditions": [
                    "_is_reaching_badly_with_vest_for_greater_than_time_threshold",
                ],
                "after": "_get_incident",
            },
            {
    ...
    def transition_state_event(
        ...
        conditions (eg. person_reach_type, person_is_associated, etc) and
        ...
        reaching_badly_with_wearing_vest_on_floor and condition satisifies.
    ...
    def _is_reaching_badly_with_vest_for_greater_than_time_threshold(
        self, **_
    ) -> bool:
        """Checks if the threshold is reached for overreaching with vest
        ...
        return (
            self.state_event_end_timestamp_ms
            - self.first_seen_not_overreaching_with_vest_timestamp_ms
            > self.max_allowed_bad_reach_with_vest_duration_ms
        )

    def _is_reaching_badly(self, person_reach_type, **_) -> bool:
        """Checks if the actor (person) is reaching badly

        Args:
            person_reach_type (_type_): used to check if person_reach_type (int)
            is bad_posture (which is 0 in structs/ergonomics)

        Returns:
            bool: True if reaching_badly (person_reach_type == 0) else False
        """
    ...
    def _is_not_associated(self, person_is_associated, **_) -> bool:
        ...
        Args:
            person_is_associated (_type_): True if associated to a PIT
    ...
    def _get_incident(self, incident_list, **_) -> None:
        incident_list.append(
            Incident(
                ...
                start_frame_relative_ms=self.first_seen_not_overreaching_with_vest_timestamp_ms,
    ...
    def on_enter_reaching_badly_with_wearing_uniform_on_floor(
        self, **_
    ) -> None:
        """This function is called whenever the transition machine enters
        the reaching_badly_with_wearing_uniform_on_floor state and we note the
...
class OverReachingWithSafetyUniformViolationMachine(BaseStateMachine):
    # State Machine Diagram:
    # https://drive.google.com/file/d/1hH3NlZ1-6BQsQqLS1kZxRIj-ZfsWKk5Z/view?usp=sharing
    ...
    NAME = "overreaching_with_uniform"
    INCIDENT_TYPE_ID = "OVERREACHING_WITH_SAFETY_UNIFORM"
    INCIDENT_TYPE_NAME = "Over Reaching with Safety Uniform"
    ...
    def get_relevant_actor(
        ...
        """Function to return the identifiers associated with the state_event_message
        ...
        Returns:
            Optional[str]: identifiers of the state_event_message
        """
        if (
