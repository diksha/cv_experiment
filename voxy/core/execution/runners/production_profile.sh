...
./bazel build //core/execution/runners:production
...
echo "Running production with args: ${all_args[*]}"

echo "Output flame graph dir: $OUTPUT_FLAME_GRAPH_DIR"

bazel-out/k8-fastbuild/bin/core/execution/runners/production "${all_args[@]}" &
pid="$!"
function cleanup() {
...
# Upload to s3
TIMESTAMP="$(date +%s)"
AWS_PATH="s3://voxel-temp/production_graph_profiling/$USER/$TIMESTAMP.svg"
