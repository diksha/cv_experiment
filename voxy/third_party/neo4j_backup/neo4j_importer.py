...
from third_party.neo4j_backup.backends import from_json


# trunk-ignore-all(flake8/D201,pylint/C0301,pylint/W1309)
# trunk-ignore-all(flake8/F541,pylint/W9008,pylint/W0707)
# trunk-ignore-all(pylint/R1720,pylint/W9011,pylint/W9012)
# trunk-ignore-all(pylint/R1714)
class Importer:
    def __init__(
        self,
        project_dir,
        ...
        self.unique_prop_key: str = from_json(
            self.project_dir / f"unique_prop_key.json"
        )
        self.labels: list = from_json(self.project_dir / "node_labels.json")
        self.relationship_types: list = from_json(
            self.project_dir / "rel_types.json"
        )
        self.constraints: list = from_json(
            self.project_dir / "constraints.json"
        )
    ...
    def _verify_is_new_db(self):
        ...
        if data:
            ...
            if self.input_yes:
                ...
                user_input = input(
                    "The database referenced is not empty, note inserting data will likely brick the "
                    "database or create duplicates. "
                    "Abort? (y/N)\n"
                )
                if user_input == "y":
                    raise UserWarning(
                        "Aborted, database referenced is not empty"
                    )
    ...
    def _apply_constraints(self):
        ...
        with self.driver.session(database=self.database) as session:
            ...
            for node_label in tqdm(
                self.labels, desc="Applying Temporary Constraints"
            ):
                constraint = (
                    # voxel changes start: cypher query deprecation fix
                    f"CREATE CONSTRAINT {node_label}_{self.unique_prop_key} IF NOT EXISTS FOR "
                    f"(m:{node_label}) REQUIRE (m.{self.unique_prop_key}) IS UNIQUE"
                    # voxel changes end
                )
    ...
    def _import_nodes_file(self, file_path):
        ...
        with self.driver.session(database=self.database) as session:
            ...
            for node in tqdm(
                from_json(file_path, compressed=self.compressed),
    ...
    def _cleanup(self):
        ...
        with self.driver.session(database=self.database) as session:
            # voxel changes start:
            #   1) comment out query as it takes up too much memory to do in db
            #   2) cypher query deprecation fix for v5.0 (WHERE EXISTS -> IS NOT NULL)
            ...
            for node_labels in tqdm(
                self.labels, desc="Removing Temporary Constraints"
            ):
            ...
            for constraint in tqdm(
                self.constraints,
                desc="Applying Actual Constraints",
                total=len(self.constraints),
            ):
                # voxel changes start: just run constraints directly
                session.run(constraint)
                # voxel changes end
