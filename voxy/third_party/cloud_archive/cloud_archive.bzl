...
"""
# This rule will download an archive from S3, check sha256, extract it, and symlink
# the provided BUILD file inside.
# File has been modified for Voxel usecases and will not be used for distribution
# Provenance: https://github.com/1e100/cloud_archive'
# trunk-ignore-all(buildifier/function-docstring,buildifier/function-docstring-args,buildifier/function-docstring-header)
...
def extract_archive(repo_ctx, local_path, strip_prefix, build_file, build_file_contents):
    ...
    # delete the archive file now that it has been unpacked
    repo_ctx.delete(local_path)
...
def cloud_archive_download(
    ...
    if provider == "minio":
        ...
        src_url = file_path
        cmd = [tool_path, "cp", "-q", src_url, repo_ctx.path(".")]
    elif provider == "google":
        tool_path = repo_ctx.which("gsutil")
        src_url = "gs://{}/{}".format(bucket, file_path)
        cmd = [tool_path, "cp", src_url, repo_ctx.path(".")]
    elif provider == "s3":
        tool_path = repo_ctx.path(Label("@aws_cli//:aws/dist/aws"))
        extra_flags = ["--profile", profile] if profile else []
        src_url = "s3://{}/{}".format(bucket, file_path)
        cmd = [tool_path] + extra_flags + ["s3", "cp", src_url, repo_ctx.path(".")]
        ...
        cmd = [tool_path, "download-file-by-name", "--noProgress", bucket, file_path, repo_ctx.path(".")]
    else:
...
# The following rules are commented out as they may not work correctly and are totally untested.
# They have not been deleted because we have no reason to assume they will not work and may be
