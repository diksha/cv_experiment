...
# whole or in part.
#
# trunk-ignore-all(pylint,flake8,semgrep)
...
def train_model(root_dir):
    ...
    wandb_key = get_secret_from_aws_secret_manager(
        "WANDB_KEY",
    )
    ...
    # opposed to before.
    optimizer = optim.SGD(
        model.fc.parameters(), lr=config.learning_rate, momentum=0.9
    )

    # Decay LR by a factor of 0.1 every 7 epochs
    scheduler = lr_scheduler.StepLR(
        optimizer, step_size=config.step, gamma=0.1
    )
    ...
    dataloaders, dataset_sizes = build_dataset(
        batch_size=config.batch_size,
        AUTOAUGMENT=config.augment,
        data_dir=root_dir,
    )
