...
def category_convertor(item):
    return 2 if item.forklift is True else ordered_classes_to_keep.index(item.category)
...
if __name__ == "__main__":
    ...
    dataset_output_path = "gs://voxel-users/common/yolo/dataset2"
    exp_output_path = "gs://voxel-users/common/yolo/dataset2"
    dataset_generator = DatasetGeneratorYOLO(
        dataset_output_path,
        ordered_classes_to_keep,
        pre_conversion_actor_filters,
        category_convertor,
    )
    ...
    dump_to_gcs(
        os.path.join(exp_output_path, "dataset2.yaml"), dataset_yaml, "text/plain"
    )
