...
func Test_ManyTestsAgainstOneDatabase(t *testing.T) {
	...
	runfile, err := bazel.Runfile("experimental/jorge/polygonpoc/lib/portalquery/portal.sql")
	require.NoError(t, err, "bazel.Runfile() to get portal.sql schema should succeed")

	portalSQL, err := os.ReadFile(runfile)
	...
	srv := NewServer(Config{Server: Server{}, Database: db.DB})
	tests := []func(t *testing.T){
		func(t *testing.T) {
			sampleOrganizationID64 := int64(sampleOrganizationID)
			expectedCameras := [1]*polygonpoc.Camera{{
				Id:             sampleID,
			...
			resp, err := srv.GetCameras(ctx, &polygonpoc.GetCamerasRequest{})
